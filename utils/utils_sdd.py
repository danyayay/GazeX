...
def get_ped_r_params(ped_r):
    ped_r_plus = ped_r + ped_r
    ped_r_plus2 = ped_r_plus * ped_r_plus
    return ped_r_plus, ped_r_plus2


def cal_em_ped(current_state, pred_vel, current_ped_nei, pred_ped_nei, delta_t, ped_r, fov_r=50, map_h=100, map_w=100):
    ped_r_plus, ped_r_plus2 = get_ped_r_params(ped_r)
...
def cal_em_envobs(current_state, pred_vel, semantic_map, ped_vision_area, delta_t, ped_r, fov_r=50, map_h=100, map_w=100):
    ...
    delta_xij[np.arange(len(ttc)), :] = ttc[np.arange(len(ttc)), None] * vij[None, ...]
...
'''
def cal_em_other(current_state, pred_vel, other_locates, delta_t, ped_r, fov_r=200, map_h=401, map_w=401):

    other_locates = np.asarray(other_locates, dtype=float)
...
def cal_em_veh(current_state, pred_vel, current_other_nei, current_other_scene, ped_vision_area, delta_t, ped_r, fov_r=200, veh_h=130, veh_w=55, map_h=401, map_w=401):
...
def cal_energymap_agent(current_state, pred_vel, semantic_map, current_other_nei, current_ped_nei, delta_t, fov_r=50, map_h=100, map_w=100):
...
def generate_velspace(curr_vel, angle_interval=9, mag_interval=3, mag_split=0.2, fov_ang=np.pi):
    curr_speed = np.linalg.norm(curr_vel, axis=1, keepdims=True)

    curr_dir = curr_vel / (curr_speed+1e-6) # (sin, cos) pair

    zero_id = np.argwhere(curr_speed==0)
    if len(zero_id) > 1:
        curr_dir[zero_id[:,0], :] = np.array([1,0])

    speed_space = np.zeros((curr_vel.shape[0], angle_interval, mag_interval, 1))
    dir_space = np.zeros((curr_vel.shape[0], angle_interval, mag_interval, 2))
    angle_split = fov_ang / (angle_interval-1)
    
    splited_angles = [angle_split*(angle_interval//2-i) for i in range(angle_interval)]
    splited_speeds = [mag_split*(mag_interval//2-i) for i in range(mag_interval)]
    ...
    for i, rot_angle in enumerate(splited_angles):
        ...
        new_dir = np.tile(new_dir, (1, mag_interval)) # N mag_interval*2
        new_dir = np.reshape(new_dir, (curr_vel.shape[0], mag_interval, -1))
        dir_space[:, i, :] = new_dir
